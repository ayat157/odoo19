...
class TierReview(models.Model):
    ...
    reviewer_group_ids = fields.Many2one(related="definition_id.reviewer_group_id")
    ...
    def _get_reviewer_fields(self):
        return ["reviewer_id", "reviewer_group_ids", "reviewer_group_ids.user_ids"] #the new code

    # return ["reviewer_id", "reviewer_group_id", "reviewer_group_id.users"] the old code
    ...
    def _compute_todo_by(self):
        ...
        for rec in self:
            todo_by = False
            if rec.reviewer_group_ids:
                todo_by = self.env._("Group %s", rec.reviewer_group_ids.name)
            else:
    ...
    def _get_reviewers(self):
        # if self.reviewer_id or self.reviewer_group_id.users: the old code
        if self.reviewer_id or self.reviewer_group_ids.user_ids:

            # return self.reviewer_id + self.reviewer_group_id.users #the oold code
            return self.reviewer_id + self.reviewer_group_ids.user_ids
