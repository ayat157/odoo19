...
class TierValidation(models.AbstractModel):
    ...
    def _search_validated(self, operator, value):
        # Accept operators used by the ORM optimizer: '=', '!=', 'in', 'not in'
        if operator not in ("=", "!=", "in", "not in"):
            raise AssertionError("Invalid domain operator")

        # Normalize value: allow booleans, iterable of booleans or string reprs
        try:
            if isinstance(value, str):
                norm_value = literal_eval(value)
            else:
                norm_value = value
        except Exception:
            norm_value = value

        if isinstance(norm_value, (list, tuple, set)):
            wants_true = True in norm_value or "True" in norm_value or "true" in norm_value
        else:
            wants_true = bool(norm_value)
    ...
    def validate_tier(self):
        ...
        if not sequences:
            raise AccessError(_("ليس لديك صلاحية لتأكيد هذا السجل."))
    ...
    def reject_tier(self):
        ...
        sequences = self._get_sequences_to_approve(self.env.user)
        if not sequences:
            raise AccessError(_("ليس لديك صلاحية لرفض هذا السجل."))
